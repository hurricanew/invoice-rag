...
- **138 tests passing**: 132 offline unit/contract tests plus 6 model-dependent integration tests (real Bedrock LLM and embedding calls), split per the spec's requirement (see [Run the tests](#run-the-tests)).
...
**Run this once before using the CLI or tests that touch retrieval** — `retrieve_finance_documents` reads `fixtures/corpus_chunks.json`, which doesn't exist on a fresh clone (it's gitignored, generated). This step makes ~70 real Bedrock Titan Embeddings calls (one per corpus chunk), takes roughly 30-40 seconds, and costs a small real amount (well under a cent — Titan Embeddings is priced per-input-token, and the whole corpus is a few thousand tokens total). Requires the same AWS credentials as everything else — see [Prerequisites](#prerequisites).

Chunks all 15 documents in `finance_rag_corpus/` (by markdown section, preserving frontmatter citation metadata) and computes a real embedding vector for each chunk, into `fixtures/corpus_chunks.json`. This file is gitignored and regenerated on demand — it is not checked into the repo. Embeddings are cached by chunk content hash (`fixtures/corpus_embeddings_cache.json`, also gitignored) so re-running ingestion after an unrelated change doesn't re-embed unchanged chunks — only content that actually changed gets a new Bedrock call.

`retrieve_finance_documents` scores relevance by real cosine similarity between the query embedding and each chunk's cached embedding, not keyword matching — see [Additional scope delivered](#additional-scope-delivered-beyond-the-baseline-spec) for a concrete example of a paraphrased query with no literal keyword overlap that still surfaces the right policy.
...
`npm test` — **132 tests, all offline**: schema validation, tool implementations, deterministic reconciliation (with exact numeric assertions), the orchestrator (with a mocked LLM and mocked retrieval), evaluation-criteria logic, the rate limiter, and dedicated fault-injection coverage (see [Additional scope delivered](#additional-scope-delivered-beyond-the-baseline-spec)). `retrieve_finance_documents` now makes a real Bedrock embedding call per invocation, so the tests exercising it directly live in `test/integration/` instead — everywhere else it's mocked to keep this suite free and fast.

`npm run test:integration` — **6 tests, real Bedrock calls, small real cost**: 3 LLM decision tests (FIN-001, FIN-002, FIN-003) asserting on actual model output, plus 3 retrieval-quality tests confirming real embedding-based cosine similarity correctly ranks relevant policy sections — including a paraphrased query with no literal keyword overlap with the target document. See [Sample output](#sample-output) below for what the LLM decision tests produce. Requires valid AWS credentials in `.env`.
...
- **Real embedding-based retrieval, not keyword matching.** `retrieve_finance_documents` originally scored relevance by term overlap as a Stage A placeholder. It now computes real Bedrock Titan Embeddings v2 vectors for every corpus chunk (cached by content hash so unchanged chunks aren't re-embedded) and scores queries by cosine similarity. Verified against a deliberately hard case: the query *"supplier payment moved to a different account without telling us first"* shares almost no vocabulary with the actual policy text ("bank account", "verification"), which a term-overlap scorer would rank near zero — real embeddings correctly surface [04_vendor_onboarding_and_bank_changes.md](finance_rag_corpus/04_vendor_onboarding_and_bank_changes.md) anyway. See [test/integration/retrieval.live.test.ts](test/integration/retrieval.live.test.ts).
...
This project was built with Claude Code (Anthropic) as a pair-programming assistant and use SPEC driven development flow — architecture discussion, code generation, test writing, live AWS deployment, and debugging (including diagnosing and fixing the Guardrail false-positive, the ESM module-hoisting test-isolation bug, and the fixture-collision bug documented in [tasks.md](tasks.md)). All design decisions were discussed and approved before implementation; the author can explain and defend every choice in this repository.
