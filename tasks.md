...
- [x] Guardrails: created via `aws bedrock create-guardrail`/`create-guardrail-version` under `rag-demo-2` — denied-topic policy for approval-bypass/verification-skip language, contextual grounding + relevance filters. Referenced by ID/version from `.env` (not committed)
