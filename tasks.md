...
- [ ] **Blocked on account setup, not code**: live Bedrock invocation currently fails with `ResourceNotFoundException: Model use case details have not been submitted for this account` — a one-time Anthropic use-case form in the Bedrock console that only an account user can submit. `scripts/try-llm-decision.ts` is ready to verify a real end-to-end call (FIN-001) once the form propagates (~15 min per AWS's own message). Model-dependent tests (grounded recommendation on a real call; adversarial doc retrieved-but-not-obeyed on a real call) deferred until this unblocks
